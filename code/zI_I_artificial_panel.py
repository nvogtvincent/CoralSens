...
_var_params = {'V': get_values(V_range, n_param),
               ...
               'w': np.array([4.]),
               'f': np.array([0.01]),
               'I': np.array([0., 0.001, 0.01]),
               'z_I': np.array([0., 1.0, 3.0]),
               'DHW_{50}': np.array([12.])}
...
plt.savefig('figures/immigration_art_' + scenario +'.pdf', bbox_inches='tight')
...
for i in range(3):
    for j in range(1, 3):
        if not j == 0:
            ax.append(f.add_subplot(gs[i, j-1]))
            submatrix = 100*(c_min[:, :, :, 0, 0, j, i, 0].T - c_min[:, :, :, 0, 0, 0, 1, 0].T).mean(dim=['site'])
            cplot = ax[-1].contourf(submatrix.V, submatrix.r_0, submatrix, levels=np.logspace(0, 2, num=26),
                                    cmap=cmr.cosmic, locator=ticker.LogLocator(), extend='min')
            
            if i == 2 and j == 1:
                ax[-1].set_xlabel('Additive genetic variance (K$^2$)', fontsize=12, x=1.1)
            ...
            if i == 0 and j == 1:
                ax[-1].set_title({'126': 'SSP1-2.6', '245': 'SSP2-4.5', '370': 'SSP3-7.0'}[scenario], fontsize=14, y=1.04, x=1.1)
            
            # Add labels
            im_text = {0: 'No immigration', 0.001: 'Moderate immigration', 0.01: 'High immigration', 0.1: 'Very high immigration'}[float(c_min[:, :, :, 0, 0, j, i, 0].T.I)]
            zc_text = r'$z_I=+$' + str(float(c_min[:, :, :, 0, 0, j, i, 0].T.z_I)) + 'C'
            textcolor = 'w' 
            ax[-1].text(0.05, 0.05, im_text, ha='left', va='bottom', fontsize=10, transform=ax[-1].transAxes, c=textcolor)
            if j > 0:
                ax[-1].text(0.05, 0.15, zc_text, ha='left', va='bottom', fontsize=10, transform=ax[-1].transAxes, c=textcolor)
...
cax.set_xlabel('Effect of immigration on coral cover (abs. %)', fontsize=12)
