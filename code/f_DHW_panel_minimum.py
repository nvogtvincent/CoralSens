...
_var_params = {'V': get_values(V_range, n_param),
               ...
               'w': np.array([4.]),
               'f': np.array([0.001, 0.01, 0.1]),
               'I': np.array([0.001]),
               'z_I': np.array([0.]),
               'DHW_{50}': np.array([4., 12., 24.])}
...
decade_min = c_decade.year[c_decade.argmin(dim='year')].mean(dim='site')
...
for i in range(3):
    for j in range(3):
        ...
        submatrix = decade_min[:, :, 0, i, 0, 0, j].T
        cplot = ax[-1].pcolormesh(submatrix.V, submatrix.r_0, submatrix, vmin=2040, vmax=2100,
                                  cmap=cmr.rainforest)
        ...
        # Add labels
        f_text = {0.001: 'Low fecundity', 0.01: 'Moderate fecunduty', 0.1: 'High fecundity'}[float(submatrix.f)]
        DHW_text = r'$DHW_{50}=$' + str(int(submatrix['DHW_{50}'])) + ' DHW'
        textcolor = 'k' if j == 2 else 'w'
        ax[-1].text(0.05, 0.05, f_text, ha='left', va='bottom', fontsize=10, transform=ax[-1].transAxes, c=textcolor)
        ax[-1].text(0.05, 0.15, DHW_text, ha='left', va='bottom', fontsize=10, transform=ax[-1].transAxes, c=textcolor)
...
cax.set_xlabel('Decade of coral cover minimum', fontsize=12)

plt.savefig('figures/f_DHW_min_' + scenario +'.pdf', bbox_inches='tight')

# EXAMPLE TIME-SERIES
f, ax = plt.subplots(1, 1, figsize=(7, 4))

# Plot 3 example time-series
ax.fill_between(output.time.dt.year, output.c[:, 29, 23, 0, 1, 0, 0, 0].min(dim='site'),
                output.c[:, 29, 23, 0, 1, 0, 0, 0].max(dim='site'), color='k', alpha=0.1,
                linewidth=0)
ax.plot(output.time.dt.year, output.c[:, 29, 23, 0, 1, 0, 0, 0].mean(dim='site'),
        label=r'4 DHW', linewidth=1, linestyle='-', c='k')

ax.fill_between(output.time.dt.year, output.c[:, 29, 23, 0, 1, 0, 0, 1].min(dim='site'),
                output.c[:, 29, 23, 0, 1, 0, 0, 1].max(dim='site'), color='darkred', alpha=0.1,
                linewidth=0)
ax.plot(output.time.dt.year, output.c[:, 29, 23, 0, 1, 0, 0, 1].mean(dim='site'),
        label=r'12 DHW', linewidth=1, linestyle='--', c='darkred')

ax.fill_between(output.time.dt.year, output.c[:, 29, 23, 0, 1, 0, 0, 2].min(dim='site'),
                output.c[:, 29, 23, 0, 1, 0, 0, 2].max(dim='site'), color='navy', alpha=0.1,
                linewidth=0)
ax.plot(output.time.dt.year, output.c[:, 29, 23, 0, 1, 0, 0, 2].mean(dim='site'),
        label=r'24 DHW', linewidth=1, linestyle=':', c='navy')
...
ax.set_title({'126': 'SSP1-2.6', '245': 'SSP2-4.5', '370': 'SSP3-7.0'}[scenario])
plt.savefig('figures/f_DHW_min_' + scenario +'_time_series.pdf', bbox_inches='tight')
