...
del output_su
del output
decade_min = c_decade.year[c_decade.argmin(dim='year')]

# # Flatten arrays for regression
decade_min_vec = decade_min.mean(dim='site').stack(param=list(_var_params.keys()))
decade_min_df = decade_min_vec.to_dataframe().reset_index(drop=True)

# Log-transform log-distributed variables
decade_min_df['V'] = np.log10(decade_min_df['V'])
decade_min_df['f'] = np.log10(decade_min_df['f'])
decade_min_df['I'] = np.log10(decade_min_df['I'])
decade_min_df['r_0'] = np.log10(decade_min_df['r_0'])

# RANDOM FOREST REGRESSION
# Prepare X, y variables
X = decade_min_df[list(_var_params.keys())]
y = decade_min_df['year']
...
plt.savefig('figures/sens_decmin_' + scenario + '.pdf', bbox_inches='tight')